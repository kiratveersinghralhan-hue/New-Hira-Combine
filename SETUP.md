# Cloudflare setup: New Hira 18.5 flat repository
...
Extract the final ZIP and remove the old repository files. This release deliberately has no folders: manually select every extracted file in GitHub's **choose your files** picker and upload them together.
...
The public website and images live at the repository root. `.assetsignore` prevents Worker source, database schema and deployment files from being published as static assets.
...
Do not add these values to GitHub or any website file.
...
- Loader or intro never finishes: confirm GitHub contains the root `app.js` and that `/app.js` returns JavaScript instead of 404.
- Worker build cannot find its entrypoint: confirm GitHub contains the root `worker.js`.
- Old layout remains visible: confirm root `index.html` contains `20260819-worker-v185`, redeploy, then hard-refresh once.
